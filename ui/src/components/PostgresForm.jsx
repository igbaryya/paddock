/**
 * Define or edit a PostgreSQL application's server: where the cluster lives and how to reach it.
 * The application is named first, like any other; this is the form its PostgreSQL box opens, the
 * way a process tile opens the process form.
 *
 * A server being defined starts from what is already on the machine: the clusters the manager finds
 * are listed above the fields, and picking one fills them in. Nothing is filled in without a click,
 * and every field stays editable after one.
 */
 ...
 */
const valuesOf = (form, forgetPassword) => ({
...
/**
 * @param {{settings: object|null, onSubmit: (values: object) => Promise<void>,
 *          onClose: () => void}} props `settings` is the application's `postgres` view — null while
 *   the server is still to be defined
 */
export default function PostgresForm({ settings, onSubmit, onClose }) {
  ...
  const submit = async (event) => {
    ...
    setSaving(true);
    try {
      await onSubmit(valuesOf(form, forgetPassword));
    } catch (err) {
  ...
  return (
    <Modal title={settings ? 'Edit PostgreSQL' : 'Define PostgreSQL'} onClose={onClose}>
      <form onSubmit={submit} noValidate>
        {!settings && <FoundClusters onPick={pickCluster} />}

        <div className="form-section">
          <h3 className="form-section-title">Cluster</h3>
          <PathField
